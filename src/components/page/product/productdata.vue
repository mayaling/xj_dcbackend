<template>
        <div class="table" v-loading="loading" element-loading-text="加载中">
            <div class="container">
                <div class="container-title">商户列表</div>
                <router-link :to="{path:'adduser'}"><el-button type="primary" style="margin-bottom:20px;">+新建商户</el-button></router-link>
                <el-table :data="tableData" border size="medium" ref="multipleTable">
                        ...
                        <el-table-column label="操作" align="center" width="110">
                            <template slot-scope="scope">
                                <router-link :to="{path:'adduser',query:{id:scope.row.id}}" style="color:#35a000;">编辑</router-link>
                            </template>
                        </el-table-column>
                    </el-table>
    ...
    <script>
        export default {
            ...
            methods: {
                getlistdata(){
                    this.$get('admins?type=4&sort=-id').then((res) => {
                        console.log(res)
                        if(res.code===1){
                ...
                handleCurrentChange(val) {
                    this.loading = true
                    this.$get('admins?type=4&sort=-id', {
                        page: val,
                    }).then((res) => {
                        if (res.code === 1) {
